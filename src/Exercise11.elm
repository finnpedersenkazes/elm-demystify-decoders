...
import Json.Decode exposing (Decoder, fail, field, int, list, map, oneOf)
...
decoderNumber : Decoder (List Int)
decoderNumber =
    map List.singleton (field "number" int)


decoderNumbers : Decoder (List Int)
decoderNumbers =
...
decoder =
    oneOf [ decoderNumber, decoderNumbers ]
